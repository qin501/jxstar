...
Jxstar.currentPage = {
	...
	**/
	...
	showHischeck: function() {
		...
		var hdcall = function(data) {
			var html = self.createHisHtml(data.root);
		...
		var whereValue = self.assignData.funId+ ';' +self.assignData.dataId;
		var whereType = 'string;string';
		var param = 'eventcode=query_data&funid=queryevent&pagetype=grid&query_funid=wf_taskhis'+
					'&where_sql='+e(whereSql)+'&where_value='+whereValue+'&where_type='+whereType;
	...
	**/
	createHisHtml:function(msgJson){
		...
		var rowTpl = new Ext.Template(
			'<li class="his-body" assginid={wf_taskhis__assign_id} dataid={wf_taskhis__data_id} funid={wf_taskhis__fun_id} taskid={wf_taskhis__task_id}>',
				'<div class="his-time">',
				...
				'<div class="his-ctn">',
					'<div class="ctn-title">',
						'<span class="main">{wf_assignhis__check_user} - {check_type}</span>',
						'<span class="det">{wf_taskhis__node_title}</span>',
						//'<span>{wf_taskhis__is_timeout}</span>',
						//'<span class="det">{wf_taskhis__start_date}</span>',
						//'<span class="det">{wf_taskhis__limit_date}</span>',
					'</div>',
					'<div class="ctn-msg">',
		...
		var rows = [];
		for (var i = 0; i < msgJson.length; i++) {
			var type = msgJson[i]['wf_assignhis__check_type'];
			msgJson[i]['check_type'] = gettype(type);
			var desc = msgJson[i]['wf_assignhis__check_desc'];
			msgJson[i]['check_desc'] = desc.replace(/\n/g, '<br>');
			var cdate = msgJson[i]['wf_assignhis__check_date'];
			msgJson[i]['check_date'] = JxUtil.shortTime(cdate);
			
			if (desc == '-') {
				msgJson[i]['check_type'] = '发起人';
				msgJson[i]['wf_taskhis__node_title'] = '';
			}
